...
1. **Identitas visual produksi diganti.** Arah seni baru = **absurdism**, **surrealism**, **Gothic Art**, **dark and blood**. Pembacaan operasional dikunci **T-038.1** (tabel di v15): deadpan menolak tropes SaaS; satu justaposisi sureal yang diam; gothic sebagai struktur (katedral/naskah), bukan stiker; darah = satu aksen wine/clot — bukan mall-goth, gore medis, NSFW, chaos acak. **Splatter horror hanya untuk selected chrome** ([ADR-030](ADR-030-selected-blood-splatter.md)). Ini mengganti kulit visual yang sekarang hidup di kode (kanvas krem, chip kuning `--chip-bg`, elevasi 3D `--elev-3d`, bahasa chrome Hess/Mazur) — bukan ganti produk, bukan halaman baru.
...
4. **Clarity tetap mengikat (UX1).** Atmosfer gothic/sureal **membingkai** klaim first viewport, tidak menyembunyikannya. ICP founder/PO SEA dan north star recall “product builder” tidak diganti. Bukan mall-goth klise, bukan chaos acak yang merusak scan. **Pengecualian selected chrome:** splatter yang mengalir ke halaman = [ADR-030](ADR-030-selected-blood-splatter.md) (T-040.6); permukaan lain tetap bukan splatter.
