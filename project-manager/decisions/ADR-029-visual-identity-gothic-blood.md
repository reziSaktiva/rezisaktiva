...
1. **Identitas visual produksi diganti.** Arah seni baru = **absurdism**, **surrealism**, **Gothic Art**, **dark and blood**. Pembacaan operasional dikunci **T-038.1** (tabel di v15): deadpan menolak tropes SaaS; satu justaposisi sureal yang diam; gothic sebagai struktur (katedral/naskah), bukan stiker; darah = satu aksen wine/clot — bukan mall-goth, gore medis, NSFW, chaos acak. **Splatter horror tidak dipakai** (ADR-030 superseded oleh [ADR-031](ADR-031-selected-chrome-outline.md): selected = outline darah). Ini mengganti kulit visual yang sekarang hidup di kode (kanvas krem, chip kuning `--chip-bg`, elevasi 3D `--elev-3d`, bahasa chrome Hess/Mazur) — bukan ganti produk, bukan halaman baru.
...
3. **Metode kerja = tanya-jawab per permukaan, lalu kode.** Tidak ada big-bang restyle. Urutan: kunci arah seni → token → chrome → overlay → halaman → gerak sistem + docs. Setiap cluster komponen dibahas dulu di chat, dikunci, baru diimplementasi. Inventaris dan gate ada di [`tasks/r1.md`](../tasks/r1.md) (**T-038** … **T-043**).
4. **Clarity tetap mengikat (UX1).** Atmosfer gothic/sureal **membingkai** klaim first viewport, tidak menyembunyikannya. ICP founder/PO SEA dan north star recall “product builder” tidak diganti. Bukan mall-goth klise, bukan chaos acak yang merusak scan. Selected chrome = outline + teks aksen ([ADR-031](ADR-031-selected-chrome-outline.md)); splatter tidak dipakai di permukaan manapun.
...
- Backlog: [`tasks/r1.md`](../tasks/r1.md) (**T-038** … **T-043**).
