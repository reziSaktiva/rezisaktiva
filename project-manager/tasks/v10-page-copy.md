...
- [ ] **T-021.3** — About / Proses Kerja: `h1`, lead, fullstack, AI, chip, “yang bisa saya bantu”, approach, values, langkah proses, note, CTA. Slot: `about.*`, `section.process`. Tulis ke `content/` (layout: T-015.2).
- [ ] **T-021.4** — Contact modal: judul, lead, email + note, availability, label form (email/pesan/placeholder/submit/sent), detail kontak, salin email, sosial, tutup. Slot: `contact.*` (bukan route `/contact`). Email nyata + URL LinkedIn/GitHub. Tulis ke `content/` (layout: T-016.2).
