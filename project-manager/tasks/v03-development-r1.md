...
* **Verifikasi (T-018.1, 2026-08-31):** dari kode produksi + copy T-021 (dev server project tidak listen di port yang dicek). **NS-1:** About offering Product / Fullstack / AI & Orchestration + Quick Info bio “Fullstack Product Builder” + orkestrasi subagent — reviewer singkat yang buka Home→About / Quick Info bisa merangkum. Home h1 lebih puitis (`Ceritamu lewat produk`) — positioning eksplisit di About/QI, bukan di hero. **NS-2:** Contact = tombol chrome + pita footer + modal (`mailto:` + form klien, tanpa harga). **Destination:** rute `/[id|en]` + `/about` + `/work` + sheet M10 + meta T-021.7. **Hybrid lean:** tidak ada `/work/[slug]`, blog, CMS, pricing. **Paritas ID/EN:** copy T-021.1–T-021.7 dikunci paralel makna. **A11y dasar:** satu `h1` per halaman; Contact/Quick Info `role="dialog"` + Escape + `trapTabKey`. **Bukan blocker exit:** foto Unsplash placeholder (alt About mengakui); T-023 CV ⏸️; `"use client"` luas di `_components/` → **T-028**.
